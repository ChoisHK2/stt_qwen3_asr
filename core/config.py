...
class Settings:
    ...
    global_queue_limit: int = 2000
    worker_concurrency: int = 1
    backpressure_slow_ratio: float = 0.6
    backpressure_pause_ratio: float = 0.9
...
def get_settings() -> Settings:
    return Settings(
        ...
        session_queue_limit=int(os.getenv("SESSION_QUEUE_LIMIT", "16")),
        global_queue_limit=int(os.getenv("GLOBAL_QUEUE_LIMIT", "2000")),
        worker_concurrency=int(os.getenv("WORKER_CONCURRENCY", "1")),
        backpressure_slow_ratio=float(os.getenv("BACKPRESSURE_SLOW_RATIO", "0.6")),
        backpressure_pause_ratio=float(os.getenv("BACKPRESSURE_PAUSE_RATIO", "0.9")),
